package com.example.mybabelreader.htmlspanner.css;
...
public class CSSCompiler {
    ...
    private static Boolean isBorderAllow(List<Selector> selectors) {
        for (Selector selector : selectors) {
            if (selector.toString().equals("p")) {
                return false;
            } else if (selector.toString().equals("table")
                    || selector.toString().equals("thread")
                    || selector.toString().equals("td")) {
                return true;
            }
        }
        return false;
    ...
    public static CompiledRule compile(Rule rule, HtmlSpanner spanner) {
        ...
        for (PropertyValue propertyValue : rule.getPropertyValues()) {
            Boolean isBorderAllow = isBorderAllow(rule.getSelectors());
            StyleUpdater updater = CSSCompiler.getStyleUpdater(propertyValue.getProperty(),
                    propertyValue.getValue(), isBorderAllow);
    ...
    public static StyleUpdater getStyleUpdater(final String key, final String value, final Boolean isBorderAllow) {
